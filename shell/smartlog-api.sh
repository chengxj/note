...
if [ -n $JAVA_HOME ];then
    ...
    echo "================"
    JAVA_HOME="/ultrapowerA/smartlog/jdk1.8.0_101"
fi
...
start() {
    ...
    if [ $psid -ne 0 ]; then
        ...
        JAVA_CMD="nohup $JAVA_HOME/bin/java $JAVA_OPTS -cp $CLASS_PATH $APP_MAINCLASS >/dev/null 2>&1 &"
        #JAVA_CMD="$JAVA_HOME/bin/java -cp $CLASS_PATH $APP_MAINCLASS"
        #echo $JAVA_CMD
